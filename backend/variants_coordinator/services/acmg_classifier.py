...
class ACMGClassifier:
    ...
    def classify_variant(
        ...
        is_null_variant = any(
            c in ["stop_gained", "frameshift_variant"] for c in consequences
        )
        ...
        if pathogenic_score >= 10:
            return (
                ...
                criteria,
                "Strong pathogenic evidence found (PVS1 + PM).",
            )
        if pathogenic_score >= 6:
            return (
                ACMG_CLASSIFICATIONS["LIKELY_PATHOGENIC"],
                criteria,
                "Moderate pathogenic evidence found.",
            )
        if benign_score >= 8:
            return (
                ACMG_CLASSIFICATIONS["BENIGN"],
                criteria,
                "Strong benign evidence found.",
            )

        return (
            ACMG_CLASSIFICATIONS["VUS"],
            criteria,
