...
class GnomADClient:
    ...
    async def batch_query_frequencies(self, variants: List[Variant]) -> Dict[str, Dict]:
        ...
        results = {}
        total_batches = sum(
            (len(chrom_vars) - 1) // 100 + 1
            for chrom_vars in variants_by_chrom.values()
        )
        current_batch = 0
        ...
        for chrom, chrom_variants in variants_by_chrom.items():
            ...
            for i in range(0, len(chrom_variants), 100):
                ...
                current_batch += 1
                logger.info(
                    f"Processing batch {current_batch}/{total_batches} for chromosome {chrom}"
                )
    ...
    async def _query_batch(
        self, chrom: str, variants: List[Variant]
    ) -> Dict[str, Dict]:
        ...
        for v in variants:
            ...
            try:
                ...
                logger.error(
                    f"Invalid position for variant {v.variant_id}: {v.pos}, error: {e}"
                )
                continue
        ...
        try:
            ...
            query_job_v3 = await loop.run_in_executor(None, self.client.query, query_v3)
            ...
            missing_variants = [
                v for v in variants if v.variant_id not in found_variants
            ]

            if missing_variants:
                logger.info(
                    f"Querying v2 for {len(missing_variants)} variants not found in v3"
                )
                ...
                if conditions_v2:  # Only query if we have valid conditions
                    ...
                    query_v2 = query_v2_template.format(
                        chrom=chrom, where_clause=where_clause_v2
                    )

                    try:
                        query_job_v2 = await loop.run_in_executor(
                            None, self.client.query, query_v2
                        )
                        rows_v2 = await loop.run_in_executor(None, query_job_v2.result)

                        for row in rows_v2:
                            key = (
                                f"{row.start_position}:{row.reference_bases}:{row.alt}"
                            )
                            if key in variant_map:
                                variant_id = variant_map[key]
                                results[variant_id] = {
                        ...
                        # Log v2 query cost
                        gb_v2 = (
                            query_job_v2.total_bytes_billed / (1024**3)
                            if query_job_v2.total_bytes_billed
                            else 0
                        )
            ...
            if not_found > 0:
                logger.info(
                    f"Chr{chrom}: {len(results)} found, {not_found} not in gnomAD"
                )

            # Log v3 query cost
            gb_v3 = (
                query_job_v3.total_bytes_billed / (1024**3)
                if query_job_v3.total_bytes_billed
                else 0
            )
