        ...
        try:
            ...
            if not all([session_id, user_id, app_name]):
                raise AgentExecutionError(
                    "Incomplete context in Firestore task document."
                )
            ...
            await task_ref.update(
                {"status": "running", "updatedAt": firestore_v1.SERVER_TIMESTAMP}
            )
            ...
            task_logger.info(
                "Loading variants from artifact...",
            ...
            try:
                ...
                if variants_artifact:
                    task_logger.info(
                        ...
                        inline_data_size=len(variants_artifact.inline_data.data)
                        if hasattr(variants_artifact, "inline_data")
            ...
            for i in range(0, total_variants, batch_size):
                ...
                task_logger.info(
                    f"Processing VEP batch {batch_num}/{total_batches}",
                ...
                cmd = [
                    ...
                    "--offline",
                    "--dir_cache",
                    "/mnt/cache",  # Using the mounted persistent disk
                    "--dir_plugins",
                    "/opt/ensembl-vep/Plugins",
                    "--assembly",
                    "GRCh38",
                    "--format",
                    "vcf",
                    ...
                    str(fork_count),
                    "--plugin",
                    "AlphaMissense,file=/app/data/AlphaMissense_hg38.tsv.gz",
                    "-o",
                    "STDOUT",
                ]
                    ...
                    try:
                        ...
                        if key in variant_map:
                            ...
                            for tc in data.get("transcript_consequences", []):
                                ...
                                if (
                                    am_score is None
                                    and am_data.get("am_pathogenicity") is not None
                                ):
                                ...
                                if (
                                    am_score is None
                                    and tc.get("am_pathogenicity") is not None
                                ):
                            ...
                            if consequences:
                                variant.info["VEP_consequence"] = list(
                                    set(consequences)
                                )
                            if impact:
                ...
                task_logger.info(
                    f"Completed VEP batch {batch_num}/{total_batches}",
                ...
                if batch_num % 25 == 0 or batch_num == total_batches:
                    ...
                    try:
                        ...
                        task_logger.warning(
                            "Failed to update Firestore progress",
            ...
            task_logger.info(
                "Preparing to save annotated variants",
            ...
            try:
                ...
                task_logger.info(
                    ...
                    inline_data_size=len(final_artifact.inline_data.data)
                    if hasattr(final_artifact, "inline_data")
                    and final_artifact.inline_data
                    else 0,
                )
                ...
                try:
                    ...
                    if verify_artifact:
                        ...
                        task_logger.error(
                            "Artifact verification failed - loaded artifact is None"
                        )
                ...
                task_logger.error("Full traceback", traceback=traceback.format_exc())
                raise
            ...
            if "session_id" in locals():
                try:
                    ...
                    task_logger.error(
                        f"Failed to update session metadata: {meta_error}"
                    )
